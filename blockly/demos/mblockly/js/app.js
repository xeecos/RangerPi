...
MBlockly.App.save = function(name) {
    ...
    if(name) {
        ...
        var w = svgOrigin[0].getBoundingClientRect().width*this.scale;
        var h = svgOrigin[0].getBoundingClientRect().width*this.scale;
...
$(function() {
    ...
    $('.restore-list').on('taphold', '.item .mask', function(e) {
        $(this).css({
            'opacity': 1
        });
